...
app.post('/webhook', async (req, res) => {
	...
	if (eventType === 'checkout.session.completed') {
		...
		const customerPageEmailLink = `${domainURL}/viewOrder/${uuid}`;
		...
		const emailText = `Dear ${name},\n\nThank you for your order! Our artists will get to work right away. ` +
			...
			`Here is a link to the customer page: ${customerPageEmailLink}`;
...
app.post('/rendition/:uuid', upload.single('upload'), async (req, res) => {
	...
	const domainURL = process.env.DOMAIN;
	const link = `${domainURL}/viewOrder/${customer_id}`
	...
	const artistPageLink = getArtistPageLink(customer_id);
	const customerPageLink = getCustomerPageLink(customer_id);

	const slackText = `The MyMoji for ${name} is ready! ${email}\n\n\t${artistPageLink}\n\n\t${customerPageLink}`;
	await publishSlackMessage(slackText);
	res.sendStatus(200);
});
...
function getCustomerPageLink(customer_id) {
	const domainURL = process.env.DOMAIN;
	return `<${domainURL}/viewOrder/${customer_id}|Customer page>`;
}

function getArtistPageLink(customer_id) {
	const domainURL = process.env.DOMAIN;
	return `<${domainURL}/artistView/${customer_id}|Artist page>`;
}
...
//To Test slack and email
// sendEmail("ataing1883@gmail.com", "test", "text");
// publishSlackMessage("this is a test message");
